...
Programme to analyse the pedestal of Helium experiment
...
from helium_deuterium_shots import shotnos,data,tLH,tHL,LHdata,HLdata
...
for i in range(len(shotnos)):
    for sig in ped_sigs:
        ...
        for j in range(len(ind[i])): # loop through time slots of L-H transition
            ...
            if not sigj: # for the case that sigj is empty because all value were 'nan', create artificial dataset
    ...
    for sig in ped_sigs:#['NE']:#
        ...
        for j in range(len(ind[i])):#[18]:#
            ...
            sig_fits.append(odr_complete(sig_fit_data,ped_tanh_odr2,diff_ped_tanh_odr2,p0[i][j][sig], xlimit=xlims_out[i][sig]
                            ,plot=False))#,title='Helium experiment shot #%d \n %s coarse profile odr fit, t=%.3f s' %(shotnos[i],sig,pedestal_data[i][sig]['time'][j]),xname='major radius [m]',yname='$n_e$ [$10^{19} m^{-3}$]'))
...
xlims_fine=[[dict(NE=[profile_fits[i]['NE'][j][0][2]-0.1,profile_fits[i]['NE'][j][0][2]+0.1],
            ...
            for j in range(len(ind[i]))] for i in range(len(shotnos))]

# take result from coarse fit as initial gues for fine fit
pedestal_fits=[{} for shot in shotnos] # for storing fit results
for i in range(len(shotnos)):#[10]: #
    for sig in ped_sigs:#['NE']:#
        sig_fits=[] # fit data for the current signal
        for j in range(len(ind[i])):
            sig_fit_data=[pedestal_data[i][sig]['radii'][j],pedestal_data[i][sig]['radii_errors'][j],pedestal_data[i][sig]['data'][j],pedestal_data[i][sig]['data_errors'][j]] # create data for fitting tool
            sig_fits.append(odr_complete(sig_fit_data,ped_tanh_odr2,diff_ped_tanh_odr2,profile_fits[i][sig][j][0], xlimit=xlims_fine[i][j][sig]
...
for i,shot in enumerate(shotnos):
    for sig in ped_sigs:
        ...
        # write to LHdata / HLdata
        LHdata[i][sig+'_ped_widthat']=dict(data=sig_LH_pwat,errors=sig_LH_pw_errat,units=data[i]['R2_CTS']['units'],time=tLH[i],resvar=sig_LH_resvarat)
        LHdata[i][sig+'_ped_widthpt']=dict(data=sig_LH_pwpt,errors=sig_LH_pw_errpt,units=data[i]['R2_CTS']['units'],time=tLH[i],resvar=sig_LH_resvarpt)
        HLdata[i][sig+'_ped_widthat']=dict(data=sig_HL_pwat,errors=sig_HL_pw_errat,units=data[i]['R2_CTS']['units'],time=tHL[i],resvar=sig_HL_resvarat)
        HLdata[i][sig+'_ped_widthpt']=dict(data=sig_HL_pwpt,errors=sig_HL_pw_errpt,units=data[i]['R2_CTS']['units'],time=tHL[i],resvar=sig_HL_resvarpt)
        
#%% write data to file
results=[shotnos,tLH,tHL,data,rubyshots,ne_R_p,Te_R_p,pe_R_p,pedestal_data,pedestal_fits,LHdata,HLdata,xlims_fine]

filename='helium_pedestal.p'
