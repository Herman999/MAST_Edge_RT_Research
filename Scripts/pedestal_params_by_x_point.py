...
ax[0].set_title(r'Pedestal Characteristics on {} point height'.format(X))
...
for i, txt in enumerate(LH_shot_list):
    ax[0].annotate(txt, (x_LH[i], ne_LH[i]))
    
for i, txt in enumerate(HL_shot_list):
    ax[0].annotate(txt, (x_HL[i], ne_HL[i]))
...
ax[1].set_ylim([28,270])
ax[1].set_xlim([0.428,0.54])
...
for i, txt in enumerate(LH_shot_list):
    ax[2].annotate(txt, (x_LH[i], pe_LH[i]))
    
for i, txt in enumerate(HL_shot_list):
    ax[2].annotate(txt, (x_HL[i], pe_HL[i]))
