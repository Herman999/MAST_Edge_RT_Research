...
ax[0].errorbar(fmt='o',x=LH_ne_average,y=LH_ne_at_ped,xerr=LH_ne_average_e,c='green',label='LH')
ax[0].errorbar(fmt='o',x=HL_ne_average,y=HL_ne_at_ped,xerr=HL_ne_average_e,c='red',label='HL')
...
ax[0].plot(neav,nefit,'--')
    
ax[0].set_ylabel(r'$N_{e}^{ped} \ [m^{-3}]$')
ax[0].legend()
ax[0].set_ylim([0,4.5e19])

# TE

ax[1].errorbar(fmt='o',x=LH_ne_average,y=LH_te_at_ped,xerr=LH_ne_average_e,yerr=LH_te_at_ped_e,c='green',label='LH')
ax[1].errorbar(fmt='o',x=HL_ne_average,y=HL_te_at_ped,xerr=HL_ne_average_e,yerr=HL_te_at_ped_e,c='red',label='HL')


(res,cov) = np.polyfit(ne_average,te_at_ped,w=1/np.sqrt(np.array(ne_average_e)**2+np.array(te_at_ped)**2),deg=1,cov=True)
neav = np.linspace(min(ne_average),max(ne_average))
nefit = res[1] + res[0] * neav
ax[1].plot(neav,nefit,'--')
ax[1].set_ylim([0,250])

ax[1].set_ylabel(r'$T_{e}^{ped} \ [eV]$')

# PE
ax[2].errorbar(fmt='o',x=LH_ne_average,y=LH_pe_at_ped,xerr=LH_ne_average_e,yerr=LH_pe_at_ped_e,c='green',label='LH')
ax[2].errorbar(fmt='o',x=HL_ne_average,y=HL_pe_at_ped,xerr=HL_ne_average_e,yerr=HL_pe_at_ped_e,c='red',label='HL')
...
neav = np.linspace(min(ne_average),max(ne_average))
nefit = res[1] + res[0] * neav
ax[2].plot(neav,nefit,'--')

ax[2].set_ylim([0,850])
ax[2].set_xlabel(r'$\overline{N_e}$')
ax[2].set_ylabel(r'$P_{e}^{ped} \ [a.u.]$')
#ax[2].legend()
